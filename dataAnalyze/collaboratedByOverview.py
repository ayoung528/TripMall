...
sql = "select * from tripmall_db.overview"
cursor.execute(sql)
...
for i, Each_row in data.iterrows():
    mydoclist.append(Each_row['overview'])
...
raw_data = {'selected_spot': [], 'recommended_spot': [], 'similarity' : []}
# raw_data 컬럼을 넣음
...
for index in range(0, first[0].size):
    ...
    sql = "select content_id from tripmall_db.overview where oid="+str((index+1))
    cursor.execute(sql)
    result = cursor.fetchall()
    ...
    for index2 in range(0, 10):
        ...
        if second.values[index2] < 0.99 and second.values[index2] > 0:
            sql = "select content_id from tripmall_db.overview where oid="+str((second.keys()[index2]+1))
            cursor.execute(sql)
            result = cursor.fetchall()
            recommended = result[0].get('content_id')
            new_row = {'selected_spot':selected, 'recommended_spot':recommended, 'similarity':second.values[index2]}
...
conn = engine.connect()
recommendAuto.to_sql(name='overviewbased', con=engine, if_exists='append')
print("finish")